...
export default function ToDoItemRow({ task, projectId, onSuccess, index }) {
  ...
  const showDeleteConfirmationToast = () => {
    toast.warning(`Are you sure you want to delete "${task.name}"?`, {
      ...
      autoClose: true, 
      closeOnClick: false,
      closeButton: <button className="text-red-500 font-light bg-white hover:bg-white/80 transition rounded-[5px] px-2" onClick={handleDelete}>Delete</button>, 
  ...
  return (
    <tr className="mb-2 bg-white/5">
      <td className="text-center w-[60px] border-r border-yellow">{index + 1}</td>
      ...
      <td className="text-center border-r border-yellow">
        <div className="flex flex-wrap gap-2 justify-center items-center">
          {task?.assignees?.map(as => {
            if(!as) return <></>;
            return (
              <span key={as.username} className="bg-purple-100 rounded-[5px] px-2 text-white">
      ...
      <td className="border-l border-b border-yellow cursor-pointer">
        <div className="grid grid-cols-2">
          <button className="w-full h-full transition bg-blue-900 hover:bg-blue-800 py-2" onClick={() => {
            alert("Edit Task");
          }}>
            <FontAwesomeIcon icon={faEdit} className="text-white" />
          </button>
          <button className="w-full h-full transition bg-red-900 hover:bg-red-800 py-2" onClick={showDeleteConfirmationToast}>
